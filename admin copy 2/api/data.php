...
if ($resource === 'footer_links') {
    if ($method === 'GET') jout(['success'=>true,'data'=>get_db()->query('SELECT * FROM footer_links ORDER BY funnel_row ASC, sort_order ASC')->fetchAll()]);
    if ($method === 'POST' && !$id) {
        $l=trim($body['label']??''); $u=trim($body['url']??'');
        if (!$l||!$u) jout(['error'=>'Label and URL required'],422);
        get_db()->prepare('INSERT INTO footer_links (label,url,funnel_row,sort_order) VALUES (?,?,?,?)')->execute([$l,$u,(int)($body['funnel_row']??1),(int)($body['sort_order']??0)]);
        jout(['success'=>true,'id'=>get_db()->lastInsertId()]);
    }
    if ($method === 'PUT' && $id) {
        get_db()->prepare('UPDATE footer_links SET label=?,url=?,funnel_row=?,sort_order=?,is_active=? WHERE id=?')
            ->execute([trim($body['label']??''),trim($body['url']??''),(int)($body['funnel_row']??1),(int)($body['sort_order']??0),(int)($body['is_active']??1),$id]);
        jout(['success'=>true]);
    }
